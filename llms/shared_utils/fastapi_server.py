...
def validate_path_safety(path_or_url, user):
    ...
    PATH_PRIVATE_UPLOAD, PATH_LOGGING = get_conf(
        'PATH_PRIVATE_UPLOAD', 'PATH_LOGGING')
    sensitive_path = None
...
def _authorize_user(path_or_url, request, gradio_app):
    ...
    if sensitive_path:
        token = request.cookies.get(
            "access-token") or request.cookies.get("access-token-unsecure")
...
def start_app(app_block, CONCURRENT_COUNT, AUTHENTICATION, PORT, SSL_KEYFILE, SSL_CERTFILE):
        ...
        async def forward_request(request: Request, method: str) -> Response:
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    raise HTTPException(
                        status_code=400, detail=f"Request to the target service failed: {str(e)}")
    ...
    requests.get(f"{app_block.local_url}startup-events",
                 verify=app_block.ssl_verify, proxies=forbid_proxies)
