...
def is_openai_api_key(key):
    ...
    if len(CUSTOM_API_KEY_PATTERN) != 0:
        ...
        API_MATCH_ORIGINAL = re.match(
            r"sk-[a-zA-Z0-9]{48}$|sk-proj-[a-zA-Z0-9]{48}$|sess-[a-zA-Z0-9]{40}$", key)
...
def select_api_key(keys, llm_model):
    ...
    if len(avail_key_list) == 0:
        raise RuntimeError(f"您提供的api-key不满足要求，不包含任何可用于{
                           llm_model}的api-key。您可能选择了错误的模型或请求源（左上角更换模型菜单中可切换openai,azure,claude,cohere等请求源）。")
