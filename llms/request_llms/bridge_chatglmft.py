...
def string_to_options(arguments):
    ...
    parser.add_argument("--llm_to_learn", type=str,
                        help="LLM model to learn", default="gpt-3.5-turbo")
    parser.add_argument("--prompt_prefix", type=str,
                        help="Prompt prefix", default='')
        ...
        while True:
            try:
                if self.chatglmft_model is None:
                    ...
                    CHATGLM_PTUNING_CHECKPOINT = get_conf(
                        'CHATGLM_PTUNING_CHECKPOINT')
                    assert os.path.exists(
                        CHATGLM_PTUNING_CHECKPOINT), "找不到微调模型检查点"
                    conf = os.path.join(
                        CHATGLM_PTUNING_CHECKPOINT, "config.json")
                    with open(conf, 'r', encoding='utf8') as f:
                        model_args = json.loads(f.read())
                    ...
                    if model_args['quantization_bit'] is not None and model_args['quantization_bit'] != 0:
                        print(f"Quantized to {
                              model_args['quantization_bit']} bit")
