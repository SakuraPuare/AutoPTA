...
class GetGLM2Handle(LocalLLMHandle):
    ...
    def load_model_and_tokenizer(self):
        ...
        LOCAL_MODEL_QUANT, device = get_conf(
            'LOCAL_MODEL_QUANT', 'LOCAL_MODEL_DEVICE')
        ...
        with ProxyNetworkActivate('Download_LLM'):
            ...
            if device == 'cpu':
                chatglm_model = AutoModel.from_pretrained(
                    _model_name_, trust_remote_code=True).float()
            else:
                chatglm_model = AutoModel.from_pretrained(
                    _model_name_, trust_remote_code=True).half().cuda()
