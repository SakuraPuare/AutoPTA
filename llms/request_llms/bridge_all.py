...
def get_token_num_gpt35(txt): return len(
    tokenizer_gpt35.encode(txt, disallowed_special=()))


def get_token_num_gpt4(txt): return len(
...
# claude家族
claude_models = ["claude-instant-1.2", "claude-2.0", "claude-2.1",
...
if "qwen-turbo" in AVAIL_LLM_MODELS or "qwen-plus" in AVAIL_LLM_MODELS or "qwen-max" in AVAIL_LLM_MODELS:   # zhipuai
    try:
        ...
        print(trimmed_format_exc())
# -=-=-=-=-=-=- 零一万物模型 -=-=-=-=-=-=-
yi_models = ["yi-34b-chat-0205", "yi-34b-chat-200k", "yi-large",
             "yi-medium", "yi-spark", "yi-large-turbo", "yi-large-preview"]
...
for model in [m for m in AVAIL_LLM_MODELS if m.startswith("one-api-")]:
    ...
    if original_model_info is not None and original_model_info.get(attribute, None) is not None:
        this_model_info.update(
            {attribute: original_model_info.get(attribute, None)})
...
if len(AZURE_CFG_ARRAY) > 0:
    for azure_model_name, azure_cfg_dict in AZURE_CFG_ARRAY.items():
        ...
        endpoint_ = azure_cfg_dict["AZURE_ENDPOINT"] + \
            f'openai/deployments/{azure_cfg_dict["AZURE_ENGINE"]
...
def predict_no_ui_long_connection(inputs: str, llm_kwargs: dict, history: list, sys_prompt: str, observe_window: list = [], console_slience: bool = False):
    ...
    if '&' not in model:
        ...
        for i in range(n_model):
            ...
            future = executor.submit(LLM_CATCH_EXCEPTION(
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                for i in range(n_model):
                    ...
                    chat_string.append(f"【{str(models[i])} 说】: <font color=\"{
                                       color}\"> {window_mutex[i][0]} </font>")
                res = '<br/><br/>\n\n---\n\n'.join(chat_string)
                # # # # # # # # # # #
                observe_window[0] = res

        t_model = threading.Thread(target=mutex_manager, args=(
            window_mutex, observe_window), daemon=True)
        t_model.start()
        ...
        for i, future in enumerate(futures):  # wait and get
            color = colors[i % len(colors)]
            return_string_collect.append(f"【{str(models[i])} 说】: <font color=\"{
                                         color}\"> {future.result()} </font>")
