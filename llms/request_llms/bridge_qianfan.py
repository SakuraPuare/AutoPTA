...
def get_access_token():
    ...
    params = {"grant_type": "client_credentials",
              "client_id": BAIDU_CLOUD_API_KEY, "client_secret": BAIDU_CLOUD_SECRET_KEY}
    access_token_cache = str(requests.post(
...
def generate_message_payload(inputs, llm_kwargs, history, system_prompt):
    ...
    if conversation_cnt:
        for index in range(0, 2*conversation_cnt, 2):
            ...
            what_gpt_answer["content"] = history[index +
                                                 1] if history[index] != "" else "Hello"
        ...
        try:
            ...
            if ('error_code' in dec) and ("max length" in dec['error_msg']):
                raise ConnectionAbortedError(
                    dec['error_msg'])  # 上下文太长导致 token 溢出
