...
class LocalLLMHandle(Process):
    ...
    def check_dependency(self):
        ...
        try:
            ...
            self.set_state(f"缺少{self.model_name}的依赖，如果要使用{self.model_name}，除了基础的pip依赖以外，您还需要运行{
                           self.cmd_to_install}安装{self.model_name}的依赖。")
            self.running = False
...
def get_local_llm_predict_fns(LLMSingletonClass, model_name, history_format='classic'):
    load_message = f"{model_name}尚未加载，加载需要一段时间。注意，取决于`config.py`的配置，{
        model_name}消耗大量的内存（CPU）或显存（GPU），也许会导致低配计算机卡死 ……"
