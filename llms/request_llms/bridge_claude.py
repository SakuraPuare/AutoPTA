...
Claude_3_Models = ["claude-3-haiku-20240307",
    ...
    while True:
        try:
            ...
            response = requests.post(endpoint, headers=headers, json=message,
                                     proxies=proxies, stream=True, timeout=TIMEOUT_SECONDS)
            break
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream=True, additional_fn=None):
    ...
    if any([llm_kwargs['llm_model'] == model for model in Claude_3_Models]) and have_recent_file:
        if inputs == "" or inputs == "空空如也的输入栏":
            inputs = "请描述给出的图片"
        # 由于没有单独的参数保存包含图片的历史，所以只能通过提示词对第几张图片进行定位
    ...
    if conversation_cnt:
        for index in range(0, 2*conversation_cnt, 2):
            ...
            what_gpt_answer["content"] = [
                {"type": "text", "text": history[index+1]}]
