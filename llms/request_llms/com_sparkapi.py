...
class Ws_Param(object):
    ...
    def create_url(self):
        ...
        authorization_origin = f'api_key="{
            self.APIKey}", algorithm="hmac-sha256", headers="host date request-line", signature="{signature_sha_base64}"'
...
class SparkRequestInstance():
    ...
    def generate(self, inputs, llm_kwargs, history, system_prompt, use_image_api=False):
        ...
        thread.start_new_thread(self.create_blocking_request,
    ...
    def create_blocking_request(self, inputs, llm_kwargs, history, system_prompt, use_image_api):
        ...
        def run(ws, *args):
            data = json.dumps(gen_params(
                ws.appid, *ws.all_args, file_manifest))
            ws.send(data)
