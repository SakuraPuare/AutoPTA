...
def msg_handle_error(llm_kwargs, chunk_decoded):
    ...
    openai_website = f' 请登录OpenAI查看详情 https://platform.openai.com/signup  api-key: `{
        api_key_encryption}`'
    error_msg = ''
    if "does not exist" in chunk_decoded:
        error_msg = f"[Local Message] Model {
        ...
        try:
            error_msg = json.dumps(json.loads(
                chunk_decoded[:6]), indent=4, ensure_ascii=False)
        except:
...
def predict(inputs: str, llm_kwargs: dict, plugin_kwargs: dict, chatbot: ChatBotWithCookies,
    ...
    log_chat(llm_model=llm_kwargs["llm_model"],
             input_str=inputs, output_str=gpt_bro_result)
