...
def make_multimodal_input(inputs, image_paths):
    ...
    for image_path in image_paths:
        ...
        inputs = inputs + \
            f'<br/><br/><div align="center"><img src="file={
                path}" base64="{base64}"></div>'
...
def append_image_if_contain_base64(inputs):
    if not contain_base64(inputs):
        ...
        pattern = re.compile(
            r'<br/><br/><div align="center"><img[^><]+></div>')
...
def predict(inputs: str, llm_kwargs: dict, plugin_kwargs: dict, chatbot: ChatBotWithCookies,
    ...
    if has_recent_image_upload:
        _inputs, image_base64_array = make_multimodal_input(
            inputs, image_paths)
    else:
    ...
    try:
        headers, payload = generate_payload(
            inputs, llm_kwargs, history, system_prompt, image_base64_array, has_multimodal_capacity, stream)
...
def generate_payload(inputs: str, llm_kwargs: dict, history: list, system_prompt: str, image_base64_array: list = [], has_multimodal_capacity: bool = False, stream: bool = True):
    ...
    try:
        print(f" {llm_kwargs['llm_model']} : {
              conversation_cnt} : {inputs[:100]} ..........")
    except:
        print('输入中可能存在乱码。')
