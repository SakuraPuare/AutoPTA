...
proxies, TIMEOUT_SECONDS, MAX_RETRY, API_ORG, AZURE_CFG_ARRAY = \
    get_conf('proxies', 'TIMEOUT_SECONDS',
             'MAX_RETRY', 'API_ORG', 'AZURE_CFG_ARRAY')
...
def decode_chunk(chunk):
    ...
    try:
        ...
        if has_content:
            has_content = (chunkjson['choices'][0]
...
def predict_no_ui_long_connection(inputs: str, llm_kwargs: dict, history: list = [], sys_prompt: str = "", observe_window: list = None, console_slience: bool = False):
    ...
    while True:
        try:
            ...
            endpoint = verify_endpoint(
                model_info[llm_kwargs['llm_model']]['endpoint'])
    ...
    while True:
        ...
        chunk_decoded, chunkjson, has_choices, choice_valid, has_content, has_role = decode_chunk(
            chunk)
...
def predict(inputs: str, llm_kwargs: dict, plugin_kwargs: dict, chatbot: ChatBotWithCookies,
    ...
    try:
        ...
        chatbot[-1] = (inputs, f"您提供的api-key不满足要求，不包含任何可用于{
                       llm_kwargs['llm_model']}的api-key。您可能选择了错误的模型或请求源。")
        # 刷新界面
    ...
    while True:
        try:
            ...
            retry_msg = f"，正在重试 ({
                retry}/{MAX_RETRY}) ……" if MAX_RETRY > 0 else ""
            # 刷新界面
    ...
    if stream:
        ...
        while True:
            try:
                ...
                # 刷新界面
                yield from update_ui(chatbot=chatbot, history=history, msg="非Cohere官方接口返回了错误:" + chunk.decode())
                return
            ...
            if chunkjson:
                try:
                    ...
                    if chunkjson['event_type'] == 'stream-end':
                        log_chat(
                            llm_model=llm_kwargs["llm_model"], input_str=inputs, output_str=gpt_replying_buffer)
                        history[-1] = gpt_replying_buffer
                        chatbot[-1] = (history[-2], history[-1])
                        # 刷新界面
                        yield from update_ui(chatbot=chatbot, history=history, msg="正常")
...
def handle_error(inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg):
    ...
    if "reduce the length" in error_msg:
        if len(history) >= 2:
            history[-1] = ""
            history[-2] = ""  # 清除当前溢出的输入：history[-2] 是本次输入, history[-1] 是本次输出
        ...
        chatbot[-1] = (chatbot[-1][0], f"[Local Message] Model {
                       llm_kwargs['llm_model']} does not exist. 模型不存在, 或者您没有获得体验资格.")
    elif "Incorrect API key" in error_msg:
        chatbot[-1] = (chatbot[-1][0],
                       "[Local Message] Incorrect API key. Cohere以提供了不正确的API_KEY为由, 拒绝服务. " + Cohere_website)
    elif "exceeded your current quota" in error_msg:
        chatbot[-1] = (chatbot[-1][0],
                       "[Local Message] You exceeded your current quota. Cohere以账户额度不足为由, 拒绝服务." + Cohere_website)
    elif "account is not active" in error_msg:
        chatbot[-1] = (chatbot[-1][0],
                       "[Local Message] Your account is not active. Cohere以账户失效为由, 拒绝服务." + Cohere_website)
    elif "associated with a deactivated account" in error_msg:
        chatbot[-1] = (chatbot[-1][0],
                       "[Local Message] You are associated with a deactivated account. Cohere以账户失效为由, 拒绝服务." + Cohere_website)
    elif "API key has been deactivated" in error_msg:
        chatbot[-1] = (chatbot[-1][0],
                       "[Local Message] API key has been deactivated. Cohere以账户失效为由, 拒绝服务." + Cohere_website)
