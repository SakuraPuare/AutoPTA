...
def get_crazy_functions():
    ...
    from crazy_functions.Latex全文润色 import Latex中文润色, Latex英文润色, Latex英文纠错
    ...
    from crazy_functions.解析项目源代码 import (
        解析一个C项目,
        解析一个C项目的头文件,
        解析一个CSharp项目,
        解析一个Golang项目,
    ...
    try:
        from crazy_functions.图片生成 import 图片修改_DALLE2, 图片生成_DALLE2, 图片生成_DALLE3
        ...
        function_plugins.update(
            {
                "图片生成_DALLE3 （先切换模型到gpt-*）": {
                    ...
                    "AdvancedArgs": True,  # 调用时，唤起高级参数输入区（默认False）
                    # 高级参数输入区的显示提示
                    "ArgsReminder": "在这里输入自定义参数「分辨率-质量(可选)-风格(可选)」, 参数示例「1024x1024-hd-vivid」 || 分辨率支持 「1024x1024」(默认) /「1792x1024」/「1024x1792」 || 质量支持 「-standard」(默认) /「-hd」 || 风格支持 「-vivid」(默认) /「-natural」",
