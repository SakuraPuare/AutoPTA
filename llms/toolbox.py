    ...
    def decorated(request: gradio.Request, cookies: dict, max_length: int, llm_model: str,
        ...
        if cookies.get('lock_plugin', None) is None:
            ...
            f_hot_reload = getattr(
                importlib.import_module(module, fn_name), fn_name)
...
def update_ui(chatbot: ChatBotWithCookies, history, msg="正常", **kwargs):  # 刷新界面
    ...
    if cookies.get("lock_plugin", None):
        ...
        chatbot_gr = gradio.update(
            value=chatbot, label=cookies.get("llm_model", ""))
...
def CatchException(f):
    ...
    def decorated(main_input: str, llm_kwargs: dict, plugin_kwargs: dict,
        ...
        try:
            ...
            chatbot_with_cookie[-1] = [chatbot_with_cookie[-1]
                                       [0], e.generate_error_html()]
            # 刷新界面
            yield from update_ui(chatbot=chatbot_with_cookie, history=history, msg=f'异常')
...
def HotReload(f):
    ...
    if get_conf("PLUGIN_HOT_RELOAD"):
        ...
        def decorated(*args, **kwargs):
            fn_name = f.__name__
            f_hot_reload = getattr(importlib.reload(
                inspect.getmodule(f)), fn_name)
...
def write_history_to_file(
    ...
    if file_fullname is None:
        if file_basename is not None:
            ...
            file_fullname = pj(
                get_log_folder(), f"GPT-Academic-{gen_time_str()}.md")
...
def on_report_generated(cookies: dict, files: List[str], chatbot: ChatBotWithCookies):
    ...
    chatbot.append(
        ["报告如何远程获取？", f"报告已经添加到右侧“文件下载区”（可能处于折叠状态），请查收。{file_links}"])
...
def have_any_recent_upload_image_files(chatbot: ChatBotWithCookies, pop: bool = False):
    ...
    if pop:
        most_recent_uploaded = chatbot._cookies.pop(
            "most_recent_uploaded", None)
    else:
        most_recent_uploaded = chatbot._cookies.get(
            "most_recent_uploaded", None)
...
def check_repeat_upload(new_pdf_path, pdf_hash):
    ...
    file_manifest = [f for f in glob.glob(
        f'{user_upload_dir}/**/{file_name}', recursive=True)]
